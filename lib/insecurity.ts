...
// FIX: Use environment variable for private key
...
// FIX: Use environment variable for HMAC key
export const hmac = (data: string) => crypto.createHmac('sha256', process.env.HMAC_KEY || 'default-hmac-key').update(data).digest('hex')
    ...
    jwt.verify(token, publicKey, (err: Error | null, decoded: any) => {
      if (err === null) {
        if (authenticatedUsers.get(token) === undefined) {
          ...
          // FIX: Set secure and HttpOnly flags for the cookie
